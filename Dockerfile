...
# package-lock.json is authored by npm 11, which writes a tree that npm 10
# rejects -- `npm ci` fails on picomatch as "not satisfied" while every local
# check passes, because `npm install` is lenient and only `npm ci` is strict.
#
# Pinning npm rather than chasing a Node tag: node:22 still ships npm 10.9, and
# node:24 happens to carry npm 11 today but is not a guarantee. The npm major
# that reads the lock file must match the one that wrote it, so state it.
ARG NPM_VERSION=11
...
# Same npm as the deps stage, so `npm run build` resolves against an identical tree.
ARG NPM_VERSION=11
RUN npm install -g "npm@${NPM_VERSION}"
