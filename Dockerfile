...
# Node 22 deliberately: package-lock.json is generated by npm 11, and
# npm 10 (shipped with node:20) resolves it differently -- `npm ci` then
# fails on a transitive version it considers unsatisfied, while every
# local check passes because `npm install` is lenient. Keep this major in
# step with the toolchain that writes the lock file.
